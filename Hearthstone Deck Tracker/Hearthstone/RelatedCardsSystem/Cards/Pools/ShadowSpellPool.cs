...
public abstract class ShadowSpellPool : DiscoverPoolCard, ICardGenerator
{
	protected override IEnumerable<Card> GetCardPool(string playerClass, GameType gt, FormatType format)
	{
		return HearthDb.Cards.Collectible.Values
			.Where(c => c is { Type: CardType.SPELL } && c.GetTag(GameTag.SPELL_SCHOOL) == (int)SpellSchool.SHADOW)
			.Select(c => new Card(c));
	}
