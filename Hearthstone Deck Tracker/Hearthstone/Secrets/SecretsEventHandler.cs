	...
	public abstract class SecretsEventHandler
	{
		...
		public void HandleFastCombat(Entity entity)
		{
			...
			if(!FastCombat.Contains(entity.CardId))
				return;
			var entites = Game.Entities.Values;
			var attacker = entites.FirstOrDefault(x => x.HasTag(GameTag.PROPOSED_ATTACKER));
			if(attacker == null)
				return;
			var defender = entites.FirstOrDefault(x => x.HasTag(GameTag.PROPOSED_DEFENDER));
			if(defender == null)
				return;
